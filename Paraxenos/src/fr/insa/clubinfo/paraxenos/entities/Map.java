...
public class Map implements Drawable {
	...
	public Map(Size size) {
		...
		Theme t = ThemeProvider.getTheme();
		backgroundPaint.setColor(t.getColor("game_bg"));
		borderPaint.setColor(t.getColor("game_border"));
	...
	public void draw(Canvas canvas) {
		...
		for (int i = shape.getEdgeCount(); --i >= 0;) {
			...
			canvas.drawLine((float) e.vertexStart.x, (float) e.vertexStart.y, (float) e.vertexEnd.x, (float) e.vertexEnd.y, borderPaint);
		}
	}
